...
import com.bnb.doggydoo.databinding.FragmentImageBinding
...
class ImageFrag : Fragment() {
    ...
    private fun getInit() {
        ...
        binding.rvImageItem.adapter = adapter
        ...
        binding.itemsswipetorefresh.setOnRefreshListener {
            binding.rvImageItem.clearAnimation()
            getNewsFeedData()
            adapter = NewsFeedDataAdapter(requireContext(), MyApp.getSharedPref().newsTypeFilter) { blockUserId, type ->
                when (type) {
                    "block" -> {
    ...
    private fun getNewsFeedData() {
        ...
        newsFeedViewModel.getByNewsFeedId("2", MyApp.getSharedPref().userId, filterType).observe(
