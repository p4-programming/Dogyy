...
import com.bnb.doggydoo.playdate.datasource.model.homepage.NearbyPlayDates
import com.bnb.doggydoo.playdate.ui.PlayDateDetailActivity
import com.bnb.doggydoo.utils.network.ApiConstant

class NearbyPetAdapter (var context: Context) :
    ListAdapter<NearbyPlayDates, NearbyPetAdapter.DogsittingDogsatViewHolder>(NearbyPetAdapter.Companion.DiffCallback()) {
    ...
    inner class DogsittingDogsatViewHolder(var binding: SingleItemDogsatBinding) :
        ...
        fun bind(detail: NearbyPlayDates) {
            if (detail.pet_age_month == "0"){
                ...
                binding.tvNameAge.text = "${detail.pet_name}, ${detail.pet_age} Years  ${detail.pet_age_month} month"
            }
